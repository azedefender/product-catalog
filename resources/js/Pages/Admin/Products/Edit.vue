<template>
  <div class="max-w-md mx-auto bg-white p-6 shadow rounded">
    <h2 class="text-xl mb-4">Редактирование товара</h2>
    <form @submit.prevent="submit" v-if="loaded">
      ...
      <button type="submit" class="w-full bg-blue-600 text-white p-2 rounded">Обновить</button>
    </form>
  </div>
</template>
...
const { getProduct, updateProduct, getCategories } = useProductApi()
const categories = ref([])
const loaded = ref(false)
const form = ref({ name: '', category_id: '', description: '', price: 0 })
...
onMounted(async () => {
  const [product, cats] = await Promise.all([getProduct(props.productId), getCategories()])
